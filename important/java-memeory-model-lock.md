...
可以看到这是个本地方法调用。这个本地方法在openjdk中依次调用的c++代码为：`unsafe.cpp`，`atomic.cpp`和`atomicwindowsx86.inline.hpp`。这个本地方法的最终实现在openjdk的如下位置：`openjdk-7-fcs-src-b147-27jun2011\openjdk\hotspot\src\oscpu\windowsx86\vm\ atomicwindowsx86.inline.hpp`（对应于windows操作系统，X86处理器）。下面是对应于intel x86处理器的源代码的片段：
